...
HashMap mmm = new CommDAO().getmap(id,"ptypes");
...
new CommDAO().commOper("update albums set ctype='"+ptname+"' where uname='"+uname+"' and ctype='"+mmm.get("ptname")+"'");
}

new CommDAO().update(request,response,"ptypes",ext,true,true); 
...
%>
<body>
  <form  action="ptypesxg.jsp?f=f&id=<%=mmm.get("id")%>"     name="f1" method="post"  onsubmit="return checkform()" >
...
var ajax = new AJAX();
ajax.post("/photoalbum/factory/checkno.jsp?table=ptypes&id=<%=mmm.get("id")%>&col="+zhwystr+"&value="+zhwystrvalue+"&checktype=zhupdate&ttime=<%=Info.getDateStr()%>") 
